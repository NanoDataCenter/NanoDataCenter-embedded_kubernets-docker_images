...
class SQLITE_Client_Support(Construct_RPC_Library):
   ...
   def list_tables(self,database):
       ...
       return_value = self.rpc_client.send_rpc_message( method="list_tables",parameters=parameters,timeout=3 )
       
       results = self.filter_result(return_value)
       return_value = []
       for i in results:
