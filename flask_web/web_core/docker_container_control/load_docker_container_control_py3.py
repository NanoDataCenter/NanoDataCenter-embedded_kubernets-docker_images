...
class Load_Docker_Processes(object):
   ...
   def assemble_url_rules(self):
       ...
       self.app.add_url_rule(self.slash_name+'/manage_processes/load_process',self.slash_name+"/docker_process_load_process",a1,methods=["POST"])
       
       # internal call
       a1 = self.auth.login_required( self.manage_processes )
       self.app.add_url_rule(self.slash_name+'/manage_processes/change_process',self.slash_name+"/docker_process_change_process",a1,methods=["POST"])
