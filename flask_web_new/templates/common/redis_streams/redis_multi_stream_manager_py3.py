...
class Redis_Multi_Stream_Manager( Redis_Stream_Base,Base_Multi_Template_Class):
   ...
   def application_page_generation(self,controller, not_used_data):
       if controller >= len(self.controllers):
           controller = len(self.controllers)-1
       self.application_generation(controller,not_used_data)
       return_value = []
       return_value.append('<script src="/static/js/plotly-latest.min.js"></script>') 
       return_value.append('<script type="text/javascript">')
       return_value.append('controller_id = '+str(controller))
       return_value.append('function change_controller(event,ui)')
       return_value.append('{')
       return_value.append('    current_page = window.location.pathname')
 
       return_value.append('    current_page = current_page+"?"+$("#controller_select")[0].selectedIndex')
       return_value.append('   window.location.href = current_page')
       return_value.append('}')
       return_value.append('   $(function () ')
       return_value.append('{')
       return_value.append('$("#controller_select").val( controller_id );')
       return_value.append('$("#controller_select").bind("change",change_controller)')   
       return_value.append('}')
       return_value.append(')')
       return_value.append("</script>")
       return_value.append('<div style="margin-top:20px"></div>')
       return_value.append('<div id="controller_tag">')
       return_value.append('<center>')
       return_value.append('<h4>Select Linux Controller</h4>')
       return_value.append('</center>')
       return_value.append('</div>')
       return_value.append('<div id="select_tag">')
       return_value.append('<center>')
       return_value.append('<select id="controller_select">')
       
       for i in range(0,len(self.controllers)):
          return_value.append('<option value="'+str(i)+'">'+self.controllers[i]+'</option>')
