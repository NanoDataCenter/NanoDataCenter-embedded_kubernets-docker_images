...
type Hybrid_Calculator_Type struct {
    apk_key string 
    cimis_url string
    station   string
    redis_string string
...
func Create_Hybrid_Calculator(    )Hybrid_Calculator{
  var return_value Hybrid_Calculator_Type   
  return_value.redis_string  = "wunder_hybrid:"+self.pws+":Normal"
    
    
  return Hybrid_Calculator_Type
}

func (r Hybrid_Calculator_Type)Compute_previous_day(){
    
   fmt.Println("compute eto")
   
   if r.eto_data.HGet(r.redis_string ) != Nil {
      fmt.Println("***************** cimis spacial eto returning")
   }
   
      if self.eto_sources.hget(r.cimis_url) != None:
       ...
       r.eto_sources.HSet("wunder_hybrid:"+self.pws+":Normal", { "eto":eto,"priority":self.priority,"status":"OK" ,"time":str(datetime.datetime.now())}       ) 
