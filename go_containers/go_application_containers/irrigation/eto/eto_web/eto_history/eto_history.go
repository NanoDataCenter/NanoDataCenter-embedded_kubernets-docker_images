...
func Generate_page_table(w http.ResponseWriter, r *http.Request){
    ...
    title := "<center><H3>Display of ETO Values in Table Form</H3></center><BR>"
    ...
    data["Title"] = "ETO HISTORICAL DATA In TABLE FORM"
...
func Generate_page_graph(w http.ResponseWriter, r *http.Request){
    ...
    title := "<center><H3>Display of ETO Values in Graph Form</H3></center><BR>"
...
func generate_html_text()string{
    keys,_ := eto_history.Find_Tag1_Entries() 
...
func generate_text_table(key string )string {
     ...
     db_data, status := eto_history.Select_after_time_stamp_desc_tag1(key, ten_year)
     if status == true {
         ...
         for index,data := range db_data{
            ...
            data  := decode_data(data.Data)
            display_list[index] = []string{key,string_date,data} 
         }
         return title+web_support.Setup_data_table("eto_history_"+key,[]string{"Station","Date","Value"},display_list)
     }
...
func decode_data( msgpack_data string)string{
