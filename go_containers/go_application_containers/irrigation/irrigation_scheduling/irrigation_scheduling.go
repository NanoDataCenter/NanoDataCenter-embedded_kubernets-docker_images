...
import (
	...
	"lacima.com/go_application_containers/irrigation/irrigation_queue_library"
	"lacima.com/go_application_containers/irrigation/irrigation_files_library"
...
func main() {
 ...
 (CF_site_node_control_cluster).Cf_set_current_row("irrigation_scheduling")
  ip := site_data_store["ip"].(string)
  port    := int(site_data_store["port"].(float64))
  (&return_value).irrigation_initialize_setup(ip,port,6)
...
func (v *Scheduling_Type)action_check_for_system_activity( system interface{},chain interface{}, parameters map[string]interface{}, event *cf.CF_EVENT_TYPE)int{
  ...
  array_element, ok := irr_files.Load_Action_File()
  if ok == true {
     v.check_action_array(&array_element)
...
func (v *Scheduling_Type)check_action_array( array_element   *irr_files.Action_File_Array_Type ){

  for _,element  := range *array_element.Array_Element {
  
     v.check_action_schedule_element(&element)
	 v.check_action_done(&element)
  }
}

func (v *Scheduling_Type)check_action_schedule_element( element *irr_files.Action_File_Type ){
 
}


func (v *Scheduling_Type)check_action_done( element *irr_files.Action_File_Type ){
   name = (*element)["name"]
    if v.determine_start_time( (*element)["start_time"],(*element)["end_time"]) == False { 
...
func (v *Scheduling_Type)check_done(element *Action_File_Type){
