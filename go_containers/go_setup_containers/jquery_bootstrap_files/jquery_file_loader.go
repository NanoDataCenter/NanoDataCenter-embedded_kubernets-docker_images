...
func main(){
	...
	file_base = "/pod_util/jquery_files"
	
	site_data = get_site_data.Get_site_data(config_file)
    graph_query.Graph_support_init(&site_data)
    search_list := []string{ "SYSTEM"}
    system_nodes := graph_query.Full_site_serach(&search_list)
    
    system_node := system_nodes[0]
    file_db_string := system_node["jquery_files"]
    file_db, err := strconv.Atoi(file_db_string)
	if err != nil {
	   panic("bad db number")
	}
	...
	(driver).FlushDB()
    fmt.Println("made it here ",file_base,file_db)
