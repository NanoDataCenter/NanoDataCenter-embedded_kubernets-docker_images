...
func Initialize_Docker_Monitor( container_search_list *[]string ,display_struct_search_list  *[]string, site_data *map[string]interface{} ){
   ...
   fmt.Println("monitored containers",Monitored_containers)
   fmt.Println("display data structures",Docker_Display_Structures)
   fmt.Println("docker status handlers",Docker_status_handlers)
   initialize_redis_display_output()
...
func Initialize_Docker_Container_Monitoring(container_search_list *[]string ,display_struct_search_list  *[]string){
  ...
  find_containers( container_search_list, &Monitored_containers )
   ...
   find_display_data_structures( display_struct_search_list ,&Docker_Display_Structures )
   initialize_display_data_structures()
...
func find_display_data_structures(display_struct_search_list  *[]string, display_structures **map[string]interface{}){
    
    *display_structures = data_handler.Construct_Data_Structures(display_struct_search_list)
}

func initialize_display_data_structures(){
    initialize_redis_display_output()
    /*for each container set field of 
       set "DOCKER_DISPLAY_DICTIONARY" to true
	   
	set "ERROR_STREAM"  of reboot map 
	                              container
								  condition
   */
