...
from redis_support_py3.redis_stream_utilities_py3 import Redis_Stream
...
class Construct_Namespace(object):
   ...
   def construct_name_space(self,topic):
       ...
       #print("name space",name_space_list)           
...
class MQTT_TO_REDIS_BRIDGE_STORE(Construct_Namespace,Redis_Stream):
   ...
   def store_mqtt_data(self,topic,mqtt_data):
       ...
       except:  # except is because data may not be encoded with msgpack
          print("*******************************exception")
          data = {}
          data["topic"] = topic
          data["timestamp"] = time.time()
